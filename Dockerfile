...
RUN postconf -e smtp_generic_maps=hash:/etc/postfix/generic ;\
    ...
    for l in sender_canonical recipient_canonical generic; do touch /etc/postfix/${l}; postmap /etc/postfix/${l}; done
...
RUN echo -e "if [ -f ~/.bash_aliases ]; then \n. ~/.bash_aliases\nfi" >>/root/.bashrc \
    ...
    && echo 'alias postgen="for l in sender_canonical recipient_canonical generic; do touch /etc/postfix/\${l}; postmap /etc/postfix/\${l}; done; postfix reload ; postqueue -f"' >>/root/.bash_aliases
