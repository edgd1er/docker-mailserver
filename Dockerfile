...
RUN echo -e "if [ -f ~/.bash_aliases ]; then \n. ~/.bash_aliases\nfi" >>/root/.bashrc \
    ...
    && echo "function getignore(){ for j in \$(fail2ban-client status | grep -oP '(?<=Jail list:)[\s]+\K.*' | tr ',' ' '); do echo \$j;fail2ban-client get \${j} ignoreip;done;}" >>/root/.bash_aliases \
